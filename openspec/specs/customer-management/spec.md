...
- GIVEN an `administrador` with a valid session WHEN they permanently delete a vehicle THEN the system MUST physically remove that `vehiculo` row from the database in the same transaction as any other `cliente` field changes, and the vehicle MUST be gone from all views and searches permanently
