# Spec: list-search-filters

This spec is created whole by `service-orders-search-and-vehicle-catalog` (WU1). It is
cross-cutting UI by precedent, not invention — `table-sorting` and
`table-bulk-actions` are already shaped this way.
