...
- [ ] 4.1 RED `modules/reminders/schedule.test.ts` — `planReminders()`: appointment = `appointmentAt - 24h`, service_due = `completedAt + 90d`; skips a channel with no contact info or opted-out (R26); drops past-due timestamps (R23).
- [ ] 4.2 GREEN `modules/reminders/schedule.ts` — implement per 4.1 + module constants.
- [ ] 4.3 RED `modules/reminders/job.test.ts` (DI fake boss+providers) — `scheduleReminder` calls `sendAfter` with a concrete Date and persists `jobId`; `runReminder` re-checks per-channel opt-out/cancelled/stale at fire time (R23, R26) before dispatch; `runReminder` on a row whose status is already `sent` is a no-op — doesn't call the provider again (ADR-8, retry-safety); `cancelReminder` flips row + removes the job.
- [ ] 4.4 GREEN `modules/reminders/job.ts` — `ensureQueue`/`scheduleReminder`/`runReminder`/`cancelReminder`/`registerReminderWorker`; `localConcurrency` (not `teamSize`); `retryLimit:3`+backoff+DLQ (R24, R25).
- [ ] 4.5 Edit `modules/service-orders/service.ts` — on `-> done` call `planReminders`+`scheduleReminder` (service_due); on appointment set/change, cancel+reschedule the appointment reminder; on `-> cancelled` call `cancelReminder` for all pending rows (R23).
- [ ] 4.6 Extend `modules/service-orders/service.test.ts` (or new `reminders/service.test.ts`) — assert reminder rows/jobs created/cancelled per 4.5 via injected fakes.
