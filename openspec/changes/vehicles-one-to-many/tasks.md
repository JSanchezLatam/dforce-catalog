...
- [x] 1.1 RED — `src/shared/db/schema.test.ts`: `vehiculo` via `getTableConfig` — columns `id`/`clienteId`/`make`/`model`/`year`/`plate`/`deactivatedAt`/`createdAt`; FK `cliente_id → cliente.id` `onDelete: "cascade"` (D1); index `vehiculo_plate_idx`; assert `cliente`'s 4 vehicle columns + `cliente_plate_idx` are still present (proves this slice does not touch `cliente`).
