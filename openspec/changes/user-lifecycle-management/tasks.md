...
- [ ] 2.1 RED `queries.test.ts` — `listActiveAdminIds()` returns only `role=administrador AND deactivatedAt IS NULL`.
- [ ] 2.2 GREEN `queries.ts`.
- [ ] 2.3 RED `service.test.ts` — `checkAdminSafety()` truth table: self-role-change, self-deactivate, last-active-admin, deactivated-admin excluded from floor, demoting an already-deactivated admin is a no-op.
- [ ] 2.4 GREEN `service.ts` — `checkAdminSafety(input): "self_role_change"|"self_deactivate"|"last_active_admin"|null` per design Decision 7.
