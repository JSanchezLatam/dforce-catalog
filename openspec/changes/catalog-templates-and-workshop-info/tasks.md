...
- [x] 6.15 Before opening this PR: run `GGA_PROVIDER=claude gga run --pr-mode --diff-only`. Run by the orchestrator after the owner accepted the size exception — **PASSED**, three non-blocking flags. One of them (the spec saying "on Cover" while the code renders a dedicated last page) led to correcting the spec, and that correction surfaced a real bug: an all-null `contact` object appended a blank page. Fixed, with the missing test. **NOT RUN** — real changed lines came to 954 (920 insertions + 34 deletions, excluding the auto-generated `meta/0010_snapshot.json`), well past the 700-line ledger cap. Per the explicit instruction not to self-grant a size exception (same discipline as WU3's stop at 797/500), work stopped here to report the overrun rather than push through GGA/PR. All 13 code/test tasks above are done and verified (`npm test` 789/789, `npx tsc --noEmit` clean, `npm run lint` 0 errors, both required live-smoke states passed).
