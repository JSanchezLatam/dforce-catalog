...
- [x] 6.4 Open PR against **`main`**, not `chore/preview-clean-output`. Task 0.1 branched off that branch so `0ad05a0` would ride along; basing the PR on it would put that commit in the BASE and therefore outside the diff — the opposite of the intent. `chore/preview-clean-output` was never pushed and exists only as the local branch point. Note the revert boundaries from proposal.md's Rollback Plan, which the accent fold reduced from three independent reverts to two.
