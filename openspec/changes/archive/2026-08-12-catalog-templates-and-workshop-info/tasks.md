...
- [x] 6.15 Before opening this PR: run `GGA_PROVIDER=claude gga run --pr-mode --diff-only`.

  **Sequence, because this checkbox changed hands.** The apply agent finished tasks 6.1–6.14 (`npm test` 789/789, tsc clean, lint 0 errors, both live-smoke states passed) and then **stopped without running GGA**: real changed lines came to 954 against the 700-line ledger cap, and per the standing instruction it reported the overrun rather than self-granting a size exception — the same discipline WU3 applied at 797/500.

  The owner accepted the exception. The orchestrator then ran GGA: **PASSED**, three non-blocking flags. One of them — the spec saying "on Cover" while the code renders a dedicated last page — led to correcting the spec, and writing the new scenario surfaced a real bug: an all-null `contact` object appended a blank page to every catalog from a workshop that had not filled contact info in. Fixed in `aa5da24` with the test that was missing.
