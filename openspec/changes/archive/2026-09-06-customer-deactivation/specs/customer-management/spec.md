...
**Restated in full.** R19 owns the route's parameter enumeration, and
`GET /api/customers` now accepts one more. R20 states the exclusion RULE; this
is the list of parameters that was left incomplete by it.

**Ordering matters**: `customer-shared-phones` also restates R19, changing that
same scenario's `phone = null` to `phone = ""`. This change archives AFTER it
and carries both edits, so archiving in branch order is correct and archiving
this one first would silently revert that fix.
