...
Deleting a vehicle MUST be a soft delete: the system MUST stamp `deactivated_at` on the `vehiculo` row rather than removing it, and MUST exclude deactivated vehicles from the customer list-view plate column, R19 search, and `CustomerPicker` display. Restoring a previously deactivated vehicle (setting `deactivated_at` back to `NULL`) MUST be supported and MUST NOT lose any of that vehicle's recorded fields.
